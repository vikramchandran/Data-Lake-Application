...
class UC2Step2(BaseStep):
    ...
    def run_res(self):
        ...
        if self.glue_instance.check_if_crawler_running(self.database):
            ...
            self.athena_instance.run_example_explicit_helper(self.tag, self.database, self.table)
            time.sleep(2)
            self.athena_instance.run_example_explicit(self.tag, self.database, self.table)
            time.sleep(1)
